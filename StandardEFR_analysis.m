...
if ~exist(outputfile)
    ...
    fprintf(fout, 'file,listener,window,Freq1,Freq2,Freq3,Freq4,Freq1NZ,Freq2NZ,Freq3NZ,Freq4NZ,rms\n');
    fclose(fout);
end
...
for i=1:nFiles(1)
    ...
    if strcmp(polarity,'pos') || strcmp(polarity,'alt')
        ...
        if exist(fullfile(fileDirectory,NegFile))
            ...
            figure
            [fftFFR, HzScale, dBfft] = myFFT(add,NEG.srate,1,trimmedFileName);
            % save figure
            ...
            fout = fopen(outputfile, 'at');
            fprintf(fout, '%s,%s,%s,%d,%d,%d,%d,%d,%d,%d,%d,%d\n', ...
                trimmedFileName,listener,window,Freq1,Freq2,Freq3,Freq4,Freq1_nz,Freq2_nz,Freq3_nz,Freq4_nz,sigrms);
            fclose(fout);
